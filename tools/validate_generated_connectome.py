#!/usr/bin/env python3
"""Independent structural validation for FlyBrain V1.13 generated data."""
...
MAGIC = b"FBC103\x00\x00"  # Binary format remains FBC103; app release is V1.13.
NODE_SIZE = 26
EDGE_SIZE = 12
HEADER_SIZE = 16
...
def main(root: Path) -> None:
    ...
    assert gradle_path.is_file(), gradle_path
    gradle_text = gradle_path.read_text()
    assert f'versionName = "{RELEASE_VERSION}"' in gradle_text, "release version mismatch: Gradle"

    assert report["dataset"] == "MaleCNS v1.0"
    assert report["flybrain_version"] == RELEASE_VERSION
    ...
    halt_role_counts = report.get("halt_role_counts", {})
    assert sum(int(halt_role_counts.get(str(i), 0)) for i in (1, 2, 3)) > 0
    ...
    for i in range(n):
        ...
        body, superclass, side, channel, motor_role, desc_role, halt_role, rf, rt, re_ = struct.unpack_from(
            "<qbbbbbbfff", data, off
        )
        assert body > 0
        ...
        assert 0 <= halt_role <= 3
        halt_nodes += halt_role > 0
        for x in (rf, rt, re_):
    ...
    assert 'FORMAT_MAGIC = "FBC103"' in meta
    assert 'FORMAT_VERSION = 103' in meta
    ...
    print(f"FBC103 independent validation for FlyBrain V{RELEASE_VERSION}: OK")
    print(f"neurons={n}")
    print(f"edges={e}")
